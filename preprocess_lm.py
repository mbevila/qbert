...
from qbert.fairseq_ext.tokenizer import Tokenizer, tokenize_line
...
def main(args):
    ...
    def make_binary_dataset(input_prefix, output_prefix, lang, num_workers):
        ...
        merge_result(
            Tokenizer.binarize(
                input_file, dict, lambda t: ds.add_item(t), offset=0, end=offsets[1], append_eos=args.append_eos,
            )
        )
        if num_workers > 1:
            pool.join()
            ...
            with open(src_file_name, "r") as src_file:
                with open(tgt_file_name, "r") as tgt_file:
                    for a, s, t in zip_longest(align_file, src_file, tgt_file):
                        si = Tokenizer.tokenize(s, src_dict, add_if_not_exist=False, append_eos=args.append_eos)
                        ti = Tokenizer.tokenize(t, tgt_dict, add_if_not_exist=False, append_eos=args.append_eos)
...
def binarize(args, filename, dict, output_prefix, lang, offset, end):
    ...
    res = Tokenizer.binarize(filename, dict, consumer, offset=offset, end=end, append_eos=args.append_eos)
    ds.finalize(dataset_dest_file(args, output_prefix, lang, "idx"))
    return res
